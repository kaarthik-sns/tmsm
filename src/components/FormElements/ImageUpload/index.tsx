...
const ImageUpload = ({ name, label, formData, formErrors, handleChange, required = false }) => {
    return (
        <div>
                ...
                <div className="w-24 h-24 flex-shrink-0 rounded-full overflow-hidden bg-gray-200 border border-gray-300">
                    {formData[name] ? (
                        <NextImage
                            ...
                            quality={100}
                            unoptimized={true}
                            className={`w-full h-full object-cover ${formErrors?.[name] ? "border-red-500" : "border-gray-300"}`}
                        />
                    ) : (
